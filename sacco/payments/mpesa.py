...
class MpesaPaymentService(BasePaymentService):
    def initiate_payment(self, phone_number, amount, description, user, account):
        cl = MpesaClient()
        try:
                ...
                if result_code == 0:  # Success
                    for item in callback_metadata:
                        if item["Name"] == "Amount":
                            ...
                            transaction.update = timezone.make_aware(datetime.strptime(str(item["Value"]), "%Y%m%d%H%M%S"))
